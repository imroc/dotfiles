...
vim.keymap.set("n", "<leader>oi", function()
  ...
  if mac_bridge.available() then
    -- SSH 环境：通过 mac-bridge 在 Mac 上打开 JetBrains Gateway
    mac_bridge.send("jb", { path = root })
    vim.notify("已触发 JetBrains Gateway: " .. root, vim.log.levels.INFO)
  else
    -- 本地：直接用 IntelliJ IDEA 打开
    job.run("idea", { args = { root } })
  end
...
vim.keymap.set("n", "<leader>odo", "<cmd>edit ~/.config/opencode/opencode.jsonc<cr>", { desc = "[P]OpenCode" })
