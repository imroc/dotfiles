...
function M.copy_absolute_path()
  copy_and_notify(buffer.pretty_path(buffer.absolute_path()))
end

function M.copy_current_directory()
  copy_and_notify(buffer.pretty_path(buffer.current_dir()))
end
...
function M.get_ai_ref_text(is_visual)
  local file_path = buffer.pretty_path(vim.fn.expand("%:p"))
  if file_path == "" then
    return nil
  end
